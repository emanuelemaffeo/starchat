...
import com.getjenny.starchat.entities.{TermIdsRequest, _}
...
object TermService extends AbstractDataService {
  ...
  def indexDefaultSynonyms(indexName: String,
    ...
    val (language, _) = Index.patternsFromIndexName(indexName)
  ...
  def indexTerm(indexName: String, terms: Terms, refresh: Int) : IndexDocumentListResult = {
    ...
    val listOfDocRes: List[IndexDocumentResult] = bulkResponse.getItems.map(x => {
      IndexDocumentResult(x.getIndex, x.getType, x.getId, x.getVersion, x.status === RestStatus.CREATED)
    }).toList
  ...
  def termsById(indexName: String,
                termsRequest: TermIdsRequest) : Terms = {
  ...
  def getTermsByIdFuture(indexName: String,
                         termsRequest: TermIdsRequest) : Future[Terms] = Future {
  ...
  private[this] def updateTerm(indexName: String, terms: Terms, refresh: Int) : UpdateDocumentsResult = {
    ...
    val listOfDocRes: List[UpdateDocumentResult] = bulkRes.getItems.map(x => {
      UpdateDocumentResult(x.getIndex, x.getType, x.getId, x.getVersion, x.status === RestStatus.CREATED)
    }).toList
  ...
  def termsDistance(indexName: String, termsReq: TermIdsRequest): Future[List[TermsDistanceRes]] = Future {
    ...
    retrievedTerms
      ...
      val v1: Vector[Double] = retrievedTerms(t1).vector.getOrElse(TextToVectorsTools.emptyVec())
      val v2: Vector[Double] = retrievedTerms(t2).vector.getOrElse(TextToVectorsTools.emptyVec())
      TermsDistanceRes(
        term1 = t1,
        term2 = t2,
        vector1 = v1,
