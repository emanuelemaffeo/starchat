    ...
    val analyzersData : List[(String, DecisionTableRuntimeItem)] = scrollResp.getHits.getHits.toList.map {
      item: SearchHit =>
        ...
        val queriesTerms: List[TextTerms] = queries.map(q => {
          ...
          queryTerms
        }).filter(_.terms.getOrElse(Terms(terms=List.empty[Term])).terms.nonEmpty)
      ...
      case Success(result) =>
        analyzerRequest.data match {
          case Some(data) =>
            ...
            decisionTableService.searchDtQueries(indexName, analyzerRequest).map(searchRes => {
              ...
              val dataInternal = AnalyzersDataInternal(
                ...
                traversedStates = data.item_list,
                extractedVariables = data.extracted_variables, data = analyzersInternalData)
              ...
              val returnData = if(evalRes.data.extractedVariables.nonEmpty || evalRes.data.traversedStates.nonEmpty) {
                ...
                Some(AnalyzersData_v4(item_list = dataInternal.traversedStates,
                  extracted_variables = dataInternal.extractedVariables))
              } else {
                Option.empty[AnalyzersData_v4]
              }
          ...
          case _ =>
            Future{
              Some(AnalyzerEvaluateResponse(build = true,
                value = 0.0, data = Option.empty[AnalyzersData_v4], build_message = "success"))
            }
        }
    }
  }
